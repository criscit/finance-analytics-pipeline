...
def build_dbt_models(
    ...
    if not tables_to_process:
        context.log.info("No tables to process - skipping model building")
        return Output(
    ...
    for table_config in tables_to_process:
        ...
        if build_result.returncode != 0:
            context.log.error("Failed to build models for %s", table_config["table_nm"])
            context.log.error("STDERR: %s", build_result.stderr)
            context.log.error("STDOUT (last 1000 chars): %s", build_result.stdout[-1000:])
            raise RuntimeError(
        ...
        context.log.info("Successfully built models for %s", table_config["table_nm"])

    context.log.info(
        "Model building completed successfully for all %s tables", len(tables_to_process)
    )
...
def build_imart_models(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    context.log.info("Building integration_marts (imart) layer...")
    ...
    if build_result.returncode != 0:
        context.log.error("Failed to build imart models")
        context.log.error("STDERR: %s", build_result.stderr)
        context.log.error("STDOUT: %s", build_result.stdout)
        raise RuntimeError(
            f"dbt build failed for imart layer: {build_result.stderr or build_result.stdout[-500:]}"
        )

    context.log.info("Successfully built imart layer")
    return Output(
