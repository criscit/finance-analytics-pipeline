...
def build_dbt_models(
    ...
    for table_config in tables_to_process:
        log.info("Building models for %s (stg, core, mart layers only)", table_config["table_nm"])
    ...
    log.info("Model building completed successfully for all %s tables", len(tables_to_process))
...
def build_imart_models(context: AssetExecutionContext) -> Output[dict[str, Any]]:
    ...
    log.info("Building integration_marts (imart) layer...")
    ...
    if build_result.returncode != 0:
        log.error("Failed to build imart models")
        log.error("STDERR: %s", build_result.stderr)
        log.error("STDOUT: %s", build_result.stdout)
        raise RuntimeError(
            f"dbt build failed for imart layer: {build_result.stderr or build_result.stdout[-500:]}"
        )

    log.info("Successfully built imart layer")
    return Output(
