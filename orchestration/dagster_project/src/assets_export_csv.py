...
import os, json, hashlib, datetime, duckdb, shutil
...
def export_csv_snapshot():
    ...
    con.execute(f"COPY (select * from {TABLE}) TO '{csv_path.as_posix()}' WITH (HEADER, DELIMITER ',')")
    ...
    return Output(
        ...
        metadata={"csv": MetadataValue.path(str(csv_path)),
